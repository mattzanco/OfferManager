...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'LoadMilestone' AND schema_id = SCHEMA_ID('offermanager'))
BEGIN
    ...
    PRINT 'Created table: offermanager.LoadMilestone'
END
ELSE
BEGIN
    PRINT 'Table offermanager.LoadMilestone already exists'
END
